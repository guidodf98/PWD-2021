...
class AbmUsuario {
  ...
  private function cargarObjeto($datosForm) {
    ...
    if (
      ...
      array_key_exists('usdeshabilitado', $datosForm) and
      array_key_exists('usvencimientocertificado', $datosForm)
    ) {
      $obj = new Usuario();
  ...
  public function buscar($condiciones) {
    ...
    if ($condiciones <> null) {
      ...
      if (isset($condiciones['usvencimientocertificado ']))
        $where .= " and usvencimientocertificado  ='{$condiciones['usvencimientocertificado ']}'";
    }
